...
def main(parser_args):
    ...
    if parser_args.mode == "inference":
        ...
        if parser_args.ens_mem is not None:
            plt_dir = plt_dir.replace(
                f"epoch_{last_or_epoch}",
                f"epoch_{last_or_epoch}_ens{parser_args.ens_mem}",
            )
        ...
        ds_out, test_info = results_from_inference(
            parser_args.model_base_dir,
            parser_args.exp_name,
            parser_args.data_dir,
            plt_dir,
            varname,
    ...
    if (
        conf_inference.get("do_feature_importance", False)
        and parser_args.mode == "inference"
    ):
        ...
        data_loader_opts = {
            "stream_mode": test_info["stream_mode"],
            "batch_size": 32,
            "predictands": test_info["all_predictands"],
            "predictors": test_info["predictors"],
            "static_predictors": test_info["static_predictors"],
            "lrepeat": False,
        ...
        all_predictors = (
            test_info["predictors"] | test_info["static_predictors"]
            if test_info["static_predictors"] is not None
            else test_info["predictors"]
        )

        _ = run_feature_importance(
            ds_test,
            conf_fi.get("predictors", all_predictors),
            varname_tar,
            test_info["trained_model"],
            test_info["data_norm"],
            conf_fi["score_name"],
            data_loader_opts,
            plt_dir_importance,
            conf_fi.get("patch_size", (4, 4)),
            parser_args.model_type,
            varname,
        )
...
if __name__ == "__main__":
    ...
    parser_inference = subparsers.add_parser(
        "inference", help="Perform inference on trained model."
    )
