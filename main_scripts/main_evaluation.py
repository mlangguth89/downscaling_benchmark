...
def main(parser_args):
    ...
    if parser_args.mode == "inference":
        raise NotImplementedError(
            "The inference mode of the postprocessing script is deprecated, use the dedicated inference script."
        )
        ...
        ds_out, model_info = results_from_file(
            parser_args.results_nc, varname, parser_args.model_name
        )
    ...
    if conf_postprocess.get("do_evaluation_time", False):
        ...
        temp_eval = TemporalEvaluation(
            varname,
            plt_dir_temporal,
            model_info,
            eval_dict=conf_postprocess.get("config_evaluation_time", None),
        )
    ...
    if conf_postprocess.get("do_spectral_analysis", False):
        ...
        run_spectral_analysis(
            ds_out,
            [f"{varname}_fcst", f"{varname}_ref"],
            plt_dir_spec,
            [model_info["model_longname"], "COSMO-REA6"],
            varname,
            unit,
    ...
    if conf_postprocess.get("do_marginal_analysis", False):
        ...
        run_marginal_analysis(
            ds_out[f"{varname}_fcst"],
            ds_out[f"{varname}_ref"],
            plt_dir_marg,
            [model_info["model_longname"], "COSMO-REA6"],
            varname,
            unit,
        ...
        logger.info(
            f"Marginal distribution analysis finished in {timer() - t0_marg:.2f}s."
        )
    ...
    if conf_postprocess.get("do_comparison_plots", False):
        ...
        offset = conf_cp.pop("offset", 0.0)

        # Note that conf_cp is parsed to the plot_comparison_maps in run_comparison_plots
        conf_cp["titles"] = [
            f"{varname.capitalize()} COSMO-REA6",
            f"{varname.capitalize()} {model_info['model_longname']}",
        ]
    ...
    if conf_postprocess.get("do_aggregate_scores", False):
        ...
        metric_dir_score_card = metric_dir_score_card.replace(
            "/plots/", "/metric_files/"
        )
        ...
        temp_eval = TemporalEvaluation(
            varname,
            os.path.join(plt_dir, "temporal_evaluation"),
            model_info,
            eval_dict=conf_postprocess.get("config_evaluation_time", None),
        )
    ...
    logger.info(
        f"Postprocessing of experiment '{parser_args.exp_name}' finished. "
...
if __name__ == "__main__":
    ...
    parser_results = subparsers.add_parser(
        "provided_results", help="Evaluate provided results."
    )
    ...
    parser_results.add_argument(
        "--derive_outdir_from_results",
        "-derive_output_dir",
        dest="derive_output_dir",
        default=False,
        action="store_true",
