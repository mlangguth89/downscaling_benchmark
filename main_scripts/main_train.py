...
print(
    "Start with importing packages at {0}".format(
...
try:
    ...
    print(
        f"{__file__}: Horovod is not installed. Distributed training is not supported."
    )
    pass
from all_normalizations import GeneralNormalizer
from handle_data_class import prepare_dataset
from model_engine import ModelEngine
from model_utils import check_config_ckpt, check_horovod, setup_horovod_devices
...
def main(parser_args):
    ...
    if js_norm:
        ...
        norm_config = {
            **ds_dict["predictors"],
            **ds_dict.get("var_tar2in", {}),
            **ds_dict.get("static_predictors", {}),
            **predictands,
        }
        ...
        if with_horovod:
            raise ValueError(
                "Normalization object must be provided when using Horovod."
            )
    ...
    tfds_train, train_info = prepare_dataset(
        datadir,
        dataset,
        ds_dict,
        hparams_dict,
    ...
    data_norm, shape_in, nsamples, tfds_train_size = (
        train_info["data_norm"],
        train_info["shape_in"],
        train_info["nsamples"],
    ...
    # validation dataset
    t0_val = timer()
    tfds_val, val_info = prepare_dataset(
        datadir,
        dataset,
        ds_dict,
        hparams_dict,
    ...
    model = model_instance(
        shape_in,
        list(train_info["all_predictands"].keys()),
        hparams_dict,
        model_savedir,
        parser_args.exp_name,
        **dist_model_config,
    )
    ...
    steps_per_epoch = ds_dict.get(
        "steps_per_epoch", int(np.ceil(nsamples / ds_dict["batch_size"]))
    )
    if "steps_per_epoch" in hparams_dict and main_process:
        print(
            f"Steps per epoch changed from {int(np.ceil(nsamples / ds_dict['batch_size']))} to {hparams_dict['steps_per_epoch']} for training."
        )
    ...
    if parser_args.ckpt_path and callable(getattr(model, "load_checkpoint", None)):
        ...
        step_ckpt = model.load_checkpoint(
            parser_args.ckpt_path,
            "h5" if parser_args.ckpt_path.endswith(".h5") else "tf",
        )
        init_epoch = int(step_ckpt / steps_per_epoch)
    elif parser_args.ckpt_path and main_process:
        print(
            f"Checkpoint path provided but model {parser_args.model} does not support loading checkpoints."
        )

    if not parser_args.ckpt_path and main_process:  # copy configuration files
        os.makedirs(model_savedir, exist_ok=True)
        filelist, filelist_new = (
            [parser_args.conf_ds.name, parser_args.conf_md.name],
        ...
        model.save_hparams_to_json(
            os.path.join(model_savedir, f"config_{parser_args.model}.json")
        )
    ...
    history = model.fit(
        ...
        validation_steps=int(8640 / ds_dict["batch_size"]),
        verbose=2 if main_process else 0,
        **model.fit_options,
    )
    ...
    if main_process:
        ...
        training_times = get_training_time_dict(
            model.fit_options["callbacks"][0].epoch_times,
        ...
        if not ttrain_load:
            ...
            print(f"Training data loading time: {ttrain_load:.2f}s.")
            print(
                f"Average throughput: {ds_obj_train.ds_proc_size / 1.0e06 / training_times['Total training time']:.3f} MB/s"
            )
        ...
        try:
            model.plot_model(
                model_savedir, show_shapes=True
            )  # , show_layer_actiavtions=True)
        except:
            plot_model(
                model,
                os.path.join(model_savedir, f"plot_{parser_args.exp_name}.png"),
                show_shapes=True,
        ...
        print(
            f"# trainable parameters: {trainable_params}, # untrainable parameters: {untrainable_params}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=argparse.FileType("r"),
        required=True,
        help="JSON-file to configure dataset to be used for training.",
    )
    parser.add_argument(
        "--json_norm_file",
        "-js_norm",
    ...
    parser.add_argument(
        "--job_id", "-id", dest="id", type=int, required=True, help="Job-id from Slurm."
    )
    parser.add_argument(
        "--checkpoint_path",
        "-ckpt_path",
