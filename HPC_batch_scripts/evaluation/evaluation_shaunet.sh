...
# var_name=glob_rad
model_name="Sha UNet"
model_name_file="sha_unet"
if [[ "${var_name}" == "ws100m" ]]; then
   exp_name=sha_unet_wind_hres_3
elif [[ "${var_name}" == "glob_rad" ]]; then
   exp_name=sha_unet_rad_hres_1
else
   exp_name=sha_unet_t2m_hres_3
fi
...
if [[ "${var_name}" == "ws100m" ]]; then
   ## only for sha wgan, sha unet, deepru, swinir and only WIND
   results_nc=${resultsdir}/downscaled_wind\_$model_name_file.nc
else
