...
class PrecipDatasetInter(torch.utils.data.IterableDataset):
    ...
    def process_netcdf(self, filenames: int = None):
        ...
        vars_in_patches_no_nan[:, self.prcp_indexes, :, :] = torch.log(
            vars_in_patches_no_nan[:, self.prcp_indexes, :, :] + torch.tensor(self.k)) - torch.log(torch.tensor(self.k))
        vars_out_pathes_no_nan = torch.log(vars_out_pathes_no_nan + torch.tensor(self.k)) - torch.log(
            torch.tensor(self.k))
