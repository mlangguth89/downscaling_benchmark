...
"""
Methods to set-up SwinIT  models incl. its building blocks.
"""

__author__ = "Ankit Patnala"
__email__ = "a.patnala@fz-juelich.de"
__date__ = "2021-XX-XX"
__update__ = "2024-02-09"
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0]) * (2 * window_size[1]), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
        ...
        relative_coords[:, :, 0] = relative_coords[:, :, 0] + torch.ones_like(relative_coords[:,:,0])*(self.window_size[0] - 1)  # shift to start from 0
        relative_coords[:, :, 1] =  relative_coords[:, :, 1] + torch.ones_like(relative_coords[:,:,1])*(self.window_size[1] - 1)
...
class SwinIR(nn.Module):
    r""" SwinIR
        ...
        The model args has been moved to a static function "get_hparams_default()"
    Model Args:
    ...
    Training Args:
        batch_size (int) : batch size furing training and validation Default 16
        lr (float) : initial learning rate Default 6e-5
        neopchs (int) : number of max epochs. Default 70
        loss (string) : loss function to be used . Default mae
        loss_weights (float | tuple(floats) : weightage of each target variable in the loss. Default [1.0,1.0] 
        weight_decay (float) : weight decay parameter of optimizer. Default 0.01
        optimizer (str) : optimizer used to boost training. Default adamw
    """
    ...
    def __init__(self,shape_in: List, varnames_tar: List, hparams: dict, savedir: str, exp_name: str = 'swinir_model'):
        ...
        num_feat = 64
        self.img_range = self.hparams["img_range"]
        if in_chans == 3:
        ...
        self.upscale = self.hparams["upscale"]
        self.upsampler = self.hparams["upsampler"]
        ...
        self.ape = self.hparams["ape"]
        self.patch_norm = self.hparams["patch_norm"]
        self.num_features = self.hparams["embed_dim"]
        self.mlp_ratio = self.hparams["mlp_ratio"]
        ...
        qk_scale = self.hparams["qk_scale"]
        attn_drop_rate = self.hparams["attn_drop_rate"]

        resi_connection = self.hparams["resi_connection"]
        
        
        self.resume_checkpoint = self.hparams["resume_checkpoint"]
    ...
    def get_hparams_dict(self,hparams_user: dict) -> dict:
        """
        ...
        unknown_keys = [key for key in hparams_user.keys() if key not in self.hparams]
        if unknown_keys:
        ...
        hparams_merged = {**self.hparams, **hparams_user}
        hparams_dict = {}
        for key in self.hparams:
            if isinstance(hparams_merged[key], type(self.hparams[key])):
                hparams_dict[key] = hparams_merged[key]
            else:
                raise TypeError("Parsed hyperparameter '{0}' must be of type '{1}', but is '{2}'"
                                .format(key, type(self.hparams[key]), type(hparams_merged[key])))
        return hparams_dict
...
class SwinIRLightning(L.LightningModule):
    ...
    def on_train_epoch_end(self):
        self.trainer.train_dataloader.dataset.datasets.update()
...
if __name__ == '__main__':
    upscale = 1 
    window_size = 8
    model = SwinIR(shape_in=[96,120,15],varnames_tar=['hsurf','2m_temp'],hparams=None,savedir=None)
    input_array = torch.randn((32,15,96,120))

    print(model(input_array).shape)
