...
from other_utils import remove_files, to_list
...
class PreprocessERA5toCREA6(PreprocessERA5toIFS):
    ...
    def __init__(
        ...
        if not os.path.isfile(tar_constfile):
            raise FileNotFoundError(
                "Could not find file with invariant data '{0}'.".format(tar_constfile)
            )
        self.constfile_tar = tar_constfile
        self.era5_sfc_vars, self.era5_ml_vars, self.era5_pl_vars = (
            self.organize_predictors(predictors)
        )
        self.crea6_sfc_vars, self.crea6_const_vars = self.organize_predictands(
            predictands
        )

        self.all_predictors = self.get_predictor_varnames(
            predictors
        )  # mlvars_era5 is a dictionary
        self.all_predictands = [
            e for e in self.crea6_sfc_vars + self.crea6_const_vars if e
        ]
    ...
    def prepare_worker(self, years: List, season: str, **kwargs):
        ...
        """
        preprocess_pystager, run_dict = super().prepare_worker(
            years, season, coarse_grid_name="crea6_", **kwargs
        )

        # correct (default) job name...
        run_dict["kwargs"] = {
            "job_name": kwargs.get("jobname", "Preproc_ERA5_to_CREA6")
        }
    ...
    def preprocess_worker(
        ...
        assert isinstance(logger, logging.Logger), (
            "logger-argument must be a logging.Logger instance"
        )
        if not os.path.isfile(invar_file_era5):
            raise FileNotFoundError(
                "File providing invariant data of ERA5-dataset'{0}' cannot be found.".format(
                    invar_file_era5
                )
            )
        ...
        if not os.path.isfile(invar_file_crea6):
            raise FileNotFoundError(
                "File providing invariant data of COSMO REA6-dataset'{0}' cannot be found.".format(
                    invar_file_crea6
                )
            )
        ...
        for year_month in year_months:
            assert isinstance(year_month, dt.datetime), (
                "All year_months-argument must be a datetime-object. Current one is of type '{0}'".format(
                    type(year_month)
                )
            )

            final_file = os.path.join(
                dirout, f"preproc_era5_crea6_{year_month.strftime('%Y-%m')}.nc"
            )
            ...
            final_file_era5, lfail, nwarn = self.preprocess_era5_in(
                dirin_era5,
                year_month,
                invar_file_era5,
                outdir_tmp,
            ...
            if lfail:
                continue  # skip month if preprocessing ERA5-data failed

            # process COSMO-REA6 doata which is already organized in monthly files
            final_file_crea6, nwarn = self.preprocess_crea6_tar(
                dirin_crea6,
                invar_file_crea6,
                grid_des_tar,
                outdir_tmp,
    ...
    def organize_predictors(predictors: dict) -> Tuple[List, dict]:
        ...
        lpred_vartypes = [
            pred_vartype in known_vartypes for pred_vartype in pred_vartypes
        ]
        if not all(lpred_vartypes):
            ...
            raise ValueError(
                "The following variables types in the predictor-dictionary are unknown: {0}".format(
                    ", ".join(unknown_vartypes)
                )
            )

        sfvars, mlvars, plvars = (
            predictors.get("sf", None),
            predictors.get("ml", None),
            predictors.get("pl", None),
        )
        ...
        if sfvars:
            try:
                sfvars = list(sfvars)
                assert all([isinstance(sfvar, str) for sfvar in sfvars]), (
                    "List of surface variables must be strings."
                )
            except Exception as err:
                raise TypeError("Surface-variables cannot be parsed into a list.")
        ...
        if mlvars:
            ...
            if not all(
                [
                    all([isinstance(v, (int, float)) for v in mlvars[k]])
                    for k in mlvars.keys()
                ]
            ):
        ...
        if plvars:
            ...
            if not all(
                [
                    all([isinstance(v, (int, float)) for v in plvars[k]])
                    for k in plvars.keys()
                ]
            ):
    ...
    def organize_predictands(predictands: dict) -> Tuple[List, List]:
        ...
        vars_2d, vars_const = (
            predictands.get("2D", None),
            predictands.get("const", None),
        )
            ...
            if not all(stat):
                ...
                raise ValueError(
                    "The following variables cannot be found in the constant file '{0}': {1}".format(
                        self.constfile_tar,
                        ",".join(list(np.array(self.crea6_const_vars)[miss_inds])),
                    )
                )
    ...
    def preprocess_era5_in(
        ...
        if self.era5_sfc_vars:
            ...
            nwarn, file2merge = self.run_preproc_func(
                self.process_era5_sf,
                [era5_dir, year_month, invar_file, dest_dir],
                {},
                logger,
                nwarn,
                max_warn,
        ...
        if self.era5_ml_vars and not lfail:
            ...
            nwarn, file2merge = self.run_preproc_func(
                self.process_era5_vl,
                [era5_dir, year_month, dest_dir, "ml"],
                {},
                logger,
                nwarn,
                max_warn,
        ...
        if self.era5_pl_vars and not lfail:
            logger.info(
                f"Process pressure level variables for {year_month_str} of ERA5."
            )
            nwarn, file2merge = self.run_preproc_func(
                self.process_era5_vl,
                [era5_dir, year_month, dest_dir, "pl"],
                {},
                logger,
                nwarn,
                max_warn,
        ...
        if filelist and not lfail:
            if len(filelist) == 1:
                # just rename file
                logger.info(
                    "Rename temporary ERA5-file to monthly netCDF-file '{0}'".format(
                        monthly_file
                    )
                )
    ...
    def preprocess_crea6_tar(
        ...
        if os.path.isfile(final_file):
            logger.info(
                "Monthly COSMO REA6-file '{0}' already exists. Ensure that data is as expected.".format(
                    final_file
                )
            )
        ...
        if self.crea6_sfc_vars:
            for var in self.crea6_sfc_vars:  # TBD: Put the following into a callable object to accumulate nwarn and filelist
                dfile_in = os.path.join(
                    dirin, "2D", var.upper(), f"{var.upper()}.2D.{date_str2}.grb"
                )
        ...
        if lfail:
            ...
            if not self.upscale_source:
                self.rename_variables(
                    final_file, {"rlat": "rlat_tar", "rlon": "rlon_tar"}
                )

        return final_file, nwarn

    def process_era5_sf(
        self, dirin_era5: str, year_month, invar_file: str, tmp_dir: str
    ) -> str:
        """
        ...
        if not os.path.isfile(invar_file):
            raise FileNotFoundError(
                "Could not find required invariant-file '{0}'".format(invar_file)
            )
        ...
        if sfvars_stat:
            ...
            if not os.path.isfile(ftmp_era5_2):  # has only to be done once
                cdo.run(
                    [invar_file, ftmp_era5_2],
                    OrderedDict([("-selname", ",".join(sfvars_stat))]),
                )
            ...
            cdo.run(
                [ftmp_era5, ftmp_era5_2, fera5_now],
                OrderedDict([("-O", ""), ("merge", "")]),
            )
    ...
    def process_era5_vl(
        self, dirin_era5: str, year_month, tmp_dir: str, vl_type: str
    ) -> str:
        """
        ...
        ftmp_list = []
        for vl in vl_list:
            ftmp_era5 = os.path.join(
                tmp_dir, f"era5_{year_month_str}_{vl_type}{vl:06d}.nc"
            )
    ...
    def process_crea6_2d(self, file_2d: str, target_dir: str, date_str: str, gdes_tar):
        ...
        if not os.path.isfile(file_2d):
            ...
            if not os.path.isfile(file_2d):
                FileNotFoundError(
                    f"Could not find required COSMO-REA6 file '{file_2d}'."
                )
    ...
    def process_crea6_const(self, target_dir: str, gdes_tar):
        ...
        if not os.path.isfile(self.constfile_tar):
            FileNotFoundError(
                f"Could not find required COSMO-REA6 file '{self.constfile_tar}'."
            )
        # retrieve variable name back from path to file
        dfile_out = os.path.join(target_dir, "const_crea6.nc")

        cdo.run(
            [self.constfile_tar, dfile_out],
            OrderedDict(
                [
                    ("selname", ",".join(const_vars)),
                    ("-sellonlatbox", lonlatbox_str),
                    ("-remapcon", gdes_tar.file),
                ]
            ),
    ...
    def remap_and_merge_data(
        ...
        if self.upscale_source:
            ...
            cdo.run(
                [file_in_coa, file_in_merge], OrderedDict([("-remapbil", fgdes_tar)])
            )
        else:
            ...
            nx, ny = (
                int(gdes_coa.grid_des_dict["xsize"]),
                int(gdes_coa.grid_des_dict["ysize"]),
            )

            cdo.run(
                [file_in_coa, file_in_merge],
                OrderedDict([("selindexbox", f"2,{nx - 1:d},2,{ny - 1:d}")]),
            )
