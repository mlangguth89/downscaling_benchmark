...
class ERA5_Data_Loader(object):
    ...
    def __init__(self, nworkers) -> None:
        ...
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.__init__.__name__}"
        )
        ...
        try:
            ...
            func_logger.error(
                f"Could not initialize CDS API client: {e} \n"
    ...
    def __call__(self, req_dict, data_dir, start, end, format, **kwargs):
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.__call__.__name__}"
        )
        ...
        if format not in self.allowed_formats:
            func_logger.warning(
                f"Unknown data format {format}. Using default format netcdf."
            )
            format = "netcdf"

        for req_key in req_list:
            if req_key == "sfc":
                out = self.download_sfc_data(
                    req_dict[req_key], data_dir, start, end, format, **kwargs
                )
            elif req_key == "ml":
                out = self.download_ml_data(
                    req_dict[req_key], data_dir, start, end, format, **kwargs
                )
            elif req_key == "pl":
                out = self.download_pl_data(
                    req_dict[req_key], data_dir, start, end, format, **kwargs
                )
            else:
                raise ValueError(
                    f"Requested key {req_key} is unknown. Valid keys are: {','.join(self.known_keys)}"
                )
    ...
    def download_sfc_data(self, varlist, data_dir, start, end, format, **kwargs):
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.download_sfc_data.__name__}"
        )
        ...
        func_logger.info(
            f"Downloading ERA5 surface data for variables {', '.join(varlist)} with {self.nworkers} workers."
        )
        ...
        for year in range(start, end + 1):
            for month in range(month_start, month_end + 1):
                ...
                func_logger.debug(
                    f"Downloading ERA5 surface data for {year}-{month:02d} to {os.path.join(data_dir, fout)}"
                )

                req_results[fout] = pool.apply_async(
                    self.cds.retrieve,
                    args=(
        ...
        func_logger.info("Finished downloading ERA5 surface data.")
    ...
    def download_ml_data(self, var_dict, data_dir, start, end, format, **kwargs):
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.download_ml_data.__name__}"
        )
        ...
        # initialize multiprocessing pool
        func_logger.info(
            f"Downloading ERA5 model-level data for variables {', '.join(vars)} with {self.nworkers} workers."
        )
        pool = Pool(self.nworkers)
        ...
        for year in range(start, end + 1):
            for month in range(month_start, month_end + 1):
                ...
                req_dict = req_dict_base.copy()
                req_dict["date"] = (
                    f"{year}-{month:02d}-01/to/{year}-{month:02d}-{last_day.day}"
                )
                fout = f"era5_ml_{year}-{month:02d}.{format}"

                func_logger.debug(
                    f"Downloading ERA5 model-level data for {year}-{month:02d} to {os.path.join(data_dir, fout)}"
                )
        ...
        func_logger.info("Finished downloading ERA5 model-level data.")

        return req_results

    def download_pl_data(self, var_dict, data_dir, start, end, format, **kwargs):
        """
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.download_pl_data.__name__}"
        )
        ...
        req_dict_base = {
            ...
            "pressure_level": [str(int(i)) for i in all_lvls],
            "day": None,
            "month": None,
            "time": [f"{h:02d}" for h in range(24)],
            "year": None,
        ...
        # initialize multiprocessing pool
        func_logger.info(
            f"Downloading ERA5 pressure-level data for variables {', '.join(vars)} with {self.nworkers} workers."
        )
        pool = Pool(self.nworkers)
        ...
        for year in range(start, end + 1):
            for month in range(month_start, month_end + 1):
                ...
                last_day = pd.Timestamp(year, month, 1) + pd.offsets.MonthEnd(1)
                req_dict["day"] = [f"{d:02d}" for d in range(1, last_day.day + 1)]
                fout = f"era5_pl_{year}-{month:02d}.{format}"

                func_logger.debug(
                    f"Downloading ERA5 pressure-level data for {year}-{month:02d} to {os.path.join(data_dir, fout)}"
                )
        ...
        func_logger.info("Finished downloading ERA5 pressure-level data.")
    ...
    def check_request_result(self, results_dict):
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.check_request_result.__name__}"
        )

        # check if all requests were successful
        stat = [
            o.get().__dict__["reply"]["state"] == "completed"
            for o in results_dict.values()
        ]

        ok = True

        if all(stat):
            func_logger.info("All requests were successful.")
        else:
            ok = False
            bad_req = np.where(np.array(stat) == False)[0]
            results_arr = np.array(list(results_dict.keys()))
            func_logger.error(
                f"The following requests were not successful: {', '.join(results_arr[bad_req])}."
            )
    ...
    def validate_request_keys(self, req_dict):
        ...
        # get local logger
        func_logger = logging.getLogger(
            f"{logger_module_name}.{self.validate_request_keys.__name__}"
        )

        # create list of requests
        req_list = []
        for req_key in req_dict.keys():
            if req_key in self.known_req_keys:
                req_list.append(req_key)
            else:
                func_logger.warning(
                    f"Unknown request key {req_key} in data request file. Skipping this request."
                )
