...
from other_utils import flatten, last_day_of_month, remove_files, to_list
from pystager_utils import PyStager
from tools_utils import CDO, NCAP2, NCEA, NCKS, NCRENAME, NCWA
...
class PreprocessERA5toIFS(AbstractPreprocessing):
    ...
    def preprocess_worker(
        ...
        sfvars, mlvars, fc_sfvars, fc_mlvars = PreprocessERA5toIFS.organize_predictors(
            predictors
        )
        all_predictors = (
            to_list(sfvars)
        ...
        for year_month in year_months:
            ...
            final_file_era5, final_file_ifs = (
                final_file.replace(".nc", "_era5.nc"),
            ...
            for date2op in dates2op:
                ...
                date_str, date_pr = (
                    date2op.strftime("%Y%m%d%H"),
                    date2op.strftime("%Y-%m-%d %H:00 UTC"),
                )
                ...
                lfail, nwarn = PreprocessERA5toIFS.preprocess_ifs_tar(
                    dirin_ifs,
                    hourly_file_ifs,
                    date2op,
                    grid_des_tar,
            ...
            PreprocessERA5toIFS.remap_and_merge_data(
                final_file_era5,
                final_file_ifs,
                final_file,
                grid_des_coarse,
    ...
    def organize_predictors(predictors: dict) -> Tuple[List, dict, List, dict]:
        ...
        if not all(lpred_vartypes):
            ...
            raise ValueError(
                "%{0}: The following variables types in the predictor-dictionary are unknown: {1}".format(
                    method, ", ".join(unknown_vartypes)
                )
            )
        ...
        if sfvars:
            if any([i is not None for i in sfvars.values()]):
                print(
                    "%{0}: Some values of sf-variables are not None, but do not have any effect.".format(
                        method
                    )
                )
            sfvars = list(sfvars)
        ...
        if fc_sfvars:
            if any([i is not None for i in fc_sfvars.values()]):
                print(
                    "%{0}: Some values of fc_sf-variables are not None, but do not have any effect.".format(
                        method
                    )
                )
    ...
    def preprocess_era5_in(
        ...
        if sfvars:
            ...
            nwarn, file2merge = AbstractPreprocessing.run_preproc_func(
                PreprocessERA5toIFS.process_sf_file,
            ...
            filelist = PreprocessERA5toIFS.manage_filemerge(
                filelist, file2merge, tmp_dir
            )
            if not file2merge:
        ...
        if mlvars and not lfail:
            ...
            nwarn, file2merge = AbstractPreprocessing.run_preproc_func(
                PreprocessERA5toIFS.process_ml_file,
        ...
        if fc_sfvars and not lfail:
            fc_file, _ = PreprocessERA5toIFS.get_fc_file(
                era5_dir, date, model="era5", prefix="sf_fc"
            )
            logger.info(
        ...
        if fc_mlvars and not lfail:
            fc_file, _ = PreprocessERA5toIFS.get_fc_file(
                era5_dir, date, model="era5", prefix="ml_fc"
            )
            logger.info(
                "Preprocess predictor from surface fcst. file '{0}' of ERA5-dataset for time step {1}".format(
                    fc_file, date_pr
    ...
    def preprocess_ifs_tar(
        ...
        nwarn, file2merge = AbstractPreprocessing.run_preproc_func(
            PreprocessERA5toIFS.process_ifs_file,
    ...
    def remap_and_merge_data(
        ...
        if l2t:
            PreprocessERA5toIFS.remap2t_and_cat(
                file_in, file_in_hres, gdes_coarse, gdes_tar
            )
            predictors.append("2t")  # to ensure subsequent renaming

        # merge input and target data
        stat = PreprocessERA5toIFS.merge_multiple_netcdf(
            [file_in_hres, file_tar], final_file
        )
    ...
    def process_sf_file(
        ...
        if not os.path.isfile(sf_file):
            raise FileNotFoundError(
                "%{0}: Could not find required surface-file '{1}'".format(
                    method, sf_file
                )
            )
    ...
    def process_ml_file(
        ...
        if not os.path.isfile(ml_file):
            raise FileNotFoundError(
                "%{0}: Could not find required multi level-file '{1}'".format(
                    method, ml_file
                )
            )
    ...
    def process_ifs_file(
        ...
        # handle date and create tmp-directory and -files
        date_str = date2op.strftime("%Y%m%d%H")
        ifs_file, fh = PreprocessERA5toIFS.get_fc_file(
            dirin_ifs, date2op, model="ifs", suffix="sfc"
        )
        tmp_dir = os.path.join(target_dir, "tmp_{0}".format(date_str))
        os.makedirs(tmp_dir, exist_ok=True)
    ...
    @staticmethod
    def remap2t_and_cat(
        infile: str, outfile: str, grid_des_coarse: str, grid_des_tar: str
    ) -> None:
        """
    ...
    def split_dyn_static(sfvars: List):
        ...
        """
        sfvars_stat = [
            sfvar for sfvar in sfvars if sfvar in PreprocessERA5toIFS.const_vars
        ]
    ...
    def get_fc_file(
        ...
        assert offset < 12, "Offset must be smaller than 12, but is {0:d}".format(
            offset
        )
        ...
        if model == "era5":
            nc_file = os.path.join(
                ...
                "{0}_{1:d}00_{2:d}_{3}.grb".format(
                    run_init.strftime("%Y%m%d"),
                    int(run_init.strftime("%H")),
                    fh,
                    prefix,
                ),
            )
    ...
    def get_varnames_from_mlvars(mlvars: dict):
        ...
        try:
            mlvars_list = list(mlvars.keys())
            if "plvls" in mlvars_list:
                mlvars_list.remove("plvls")
            mlvarnames = [
                "{0}{1}".format(var, int(int(plvl.lstrip("p")) / 100))
                for var in mlvars_list
                for plvl in mlvars[var]
            ]
    ...
    def retrieve_plvls(mlvars_dict):
        ...
        if len(lvls) != len(plvls):
            raise ValueError(
                "Could not retrieve all parsed level imformation. Check the folllowing: {0}".format(
                    ", ".join(lvls)
                )
            )
    ...
    def check_season(season: str) -> List:
        ...
        if season == "DJF":
            ...
            raise ValueError(
                "%{0}: Parsed season-string '{1}' is unknown. Handle one of the following known ones: {1}".format(
                    method, ", ".join(known_seasons)
                )
            )
