...
def results_from_inference_lightning(model_base_dir: Union[Path, str], exp_name: str, data_dir: Union[Path, str], out_dir: Union[Path, str],
    ...
    trained_model, model_info = get_trained_model_lightning(model_base, exp_name, last_or_epoch, model_type)
    ...
    tfds_test, test_info = prepare_torch_dataset(data_dir, dataset, ds_dict, model_info["hparams_dict"], "test", norm_obj=data_norm, 
    ...
    trainer = Trainer(
            ...
            devices=1,
            accelerator="cuda")
    y_pred = trainer.predict(trained_model,tfds_test)
...
def get_trained_model_lightning(model_base: Union[Path, str], exp_name: str, last_or_epoch: Union[str, int], model_type: str = None):
    """
    ...
    if isinstance(last_or_epoch, str):
        add_str=f"{last_or_epoch}"
    else:
        add_str=""

    model_dir = Path(model_base).joinpath(f"{exp_name}")
    ...
    model_dir = Path(model_dir).joinpath(f"{add_str}")
    func_logger.info(f"Load model '{exp_name}' from {model_dir}")
    trained_model = SwinIRLightning.load_from_checkpoint(checkpoint_path=model_dir,shape_in=None, varnames_tar=vars_tar_dummy, hparams=hparams_dict, savedir=model_base, exp_name=exp_name)
...
def run_feature_importance_lightning(ds: xr.Dataset, predictors: list_or_str, varname_tar: str, model, norm, score_name: str,
    ...
    feature_scores = feature_importance_lightning(ds, predictors, varname_tar, model, norm, score_name, data_loader_opt, 
...
def feature_importance_lightning(ds: xr.Dataset, predictors: list_or_str, varname_tar: str, model, norm, score_name: str,
    ...
    for var in predictors:
        ...
        y_pred = trainer.predict(model, tfds_test) 
