...
from aux_funcs import (
    ...
    manage_filemerge,
    organize_predictands,
    organize_predictors,
    preprocess_crea6_tar,
    preprocess_era5_in,
...
class PreprocessERA5toCREA6(PreprocessERA5toIFS):
    ...
    def worker(
        ...
        kwargs = {
            ...
            "invar_file_target": pathes.invars_target, # only for era5 to crea6
    ...
    def preprocess_worker(
        ...
        sfvars_era5, mlvars_era5, fc_sfvars_era5, fc_mlvars_era5 = organize_predictors(
            predictors
        )

        all_predictors = (
            to_list(sfvars_era5)
            + get_varnames_from_mlvars(mlvars_era5)
            + to_list(fc_sfvars_era5)
            + get_varnames_from_mlvars(fc_mlvars_era5)
        )
        ...
        for year_month in year_months:
            ...
            _ = check_crea6_files(
                dirin_crea6, invar_file_crea6, subdir, sfvars_crea6, const_vars_crea6
            )
            dest_dir = os.path.join(dirout, "netcdf_data", year_str, subdir)
            final_file_era5 = os.path.join(
                dest_dir, "preproc_era5_{0}.nc".format(subdir)
            )
            ...
            if not os.path.isdir(dir_curr_era5):
                err_mess = "Could not find directory for ERA5-data '{0}'".format(
                    dir_curr_era5
                )
            ...
            if not os.path.isfile(final_file_era5):
                ...
                for date2op in dates2op:
                    ...
                    if date2op <= dt.datetime.strptime("20060101 12", "%Y%m%d %H"):
                        continue
                    ...
                    lfail, nwarn = preprocess_era5_in(
                        dirin_era5,
                        invar_file_era5,
                        hourly_file_era5,
                        date2op,
                    ...
                    if not lfail:
                        continue  # skip day if preprocessing ERA5-data failed
                ...
                logger.info(
                    "Merge all hourly files to monthly datafile '{0}'".format(
                        final_file_era5
                    )
                )
                ...
                logger.info(
                    "Monthly ERA5-file '{0}' already exists. Ensure that data is as expected.".format(
                        final_file_era5
                    )
                )

            # process COSMO-REA6 doata which is already organized in monthly files
            final_file_crea6, nwarn = preprocess_crea6_tar(
                dirin_crea6,
                invar_file_crea6,
                grid_des_tar,
                dest_dir,
    ...
    def preprocess_crea6_tar(
        ...
        if vars_2d:
            for var in vars_2d:
                # TBD: Put the following into a callable object to accumulate nwarn and filelist
        ...
        if const_vars and not lfail:
            nwarn, file2merge = run_preproc_func(
                cls.process_const_file,
                [invar_file, dest_dir, const_vars, date_str, gdes_tar],
                {},
                logger,
                nwarn,
                max_warn,
        ...
        if lfail:
            ...
            for const_var in const_vars:
                ...
                cls.ncks.run(
                    [final_file, final_file],
                    OrderedDict([("-O", ""), ("-x", ""), ("-v", const_var)]),
                )
                cls.ncrename.run(
                    [final_file], OrderedDict([("-v", f"{const_var}z,{const_var}")])
                )
    ...
    @classmethod
    def process_const_file(
        cls, const_file: str, target_dir: str, const_vars: List, date_str: str, gdes_tar
    ):
        """
