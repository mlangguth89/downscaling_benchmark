...
class ERA5_Data_Loader(object):
    ...
    def __call__(self, req_dict, data_dir, start, end, format, **kwargs):
        ...
        for req_key in req_list:
            if req_key == "sfc":
                ...
                out = self.download_pl_data(req_dict[req_key], data_dir, start, end, format, **kwargs)
            else:
                raise ValueError(f"Requested key {req_key} is unknown. Valid keys are: {','.join(self.known_keys)}")
    ...
    def download_pl_data(self, var_dict, data_dir, start, end, format, **kwargs):
        ...
        """
        # get local logger
        func_logger = logging.getLogger(f"{logger_module_name}.{self.download_pl_data.__name__}")
