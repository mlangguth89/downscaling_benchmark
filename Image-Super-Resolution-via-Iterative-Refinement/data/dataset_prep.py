    ...
    def __iter__(self):
        ...
        for bidx in range(iter_start, iter_end):
            ...
            for jj in range(self.batch_size):
                ...
                x_sr = F.interpolate(x, [y.shape[2], y.shape[2]], mode = 'bilinear', align_corners = True)

                self.idx += 1
            yield {'LR': x, 'HR': y, "SR": x_sr, "idx": cidx, "T":t}


def run():
    data_loader = PrecipDatasetInter(file_path="/Users/gongbing/PycharmProjects/downscaling_maelstrom/data")
    print("created data_loader")
    for batch_idx, train_data in enumerate(data_loader):
        inputs = train_data["LR"]
        target = train_data["HR"]
        Reinputs = train_data["SR"]
        idx = train_data["idx"]
        times = train_data["T"]
